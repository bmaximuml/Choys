...
def test_run_google_maps_distance_matrix(db_session):
    ...
    try:
        ...
        assert False, f'run_google_maps_distance_matrix threw an error: {str(e)}'

    new_locations = db_session.query(Location).all()

    for loc in new_locations:
        assert len(loc.rental_data_rel) in [distance_matrix_counts[loc.name], distance_matrix_counts[loc.name] + 1]
