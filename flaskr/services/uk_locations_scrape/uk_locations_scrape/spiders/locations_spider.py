"""Scrapy spider to scrape home.co.uk and associated functions"""
...
def catch_and_zero(response, css, regex=None):
    ...
    This is used in some cases which may cause 'None's to be reported as
    '0's."""
    try:
        if regex is None:
...
class LocationsSpider(Spider):
    """Scrapy spider to scrape rental data home.co.uk"""
    name = "locations"
    start_urls = ['https://www.home.co.uk/for_rent/current_rents_by_town.htm']

    def parse(self, response):
        logger = logging.getLogger()
        title = response.css('title::text').get()
        if 'Market Rents' in title:
            for list_item in response.css('li a::attr(href)').getall():
                yield response.follow(list_item, callback=self.parse)
        elif 'Market Rent Summary' in title:
            current_location = response.css('title::text').re(
                '(?!Home\\.co\\.uk:)[A-Za-z\\- ]*(?=Market Rent Summary)'
            )[0].strip()
