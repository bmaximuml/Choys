...
class Location(db.Model):
    ...
    rental_data_rel = db.relationship(
        'RentalData',
        back_populates='location_rel',
        lazy='joined',
        cascade='all, delete-orphan'
    )
    distance_matrix_data_rel = db.relationship(
        'DistanceMatrixData',
        back_populates='location_rel',
        lazy='joined',
        cascade='all, delete-orphan'
    )
    scores_rel = db.relationship(
        'Scores',
        back_populates='location_rel',
        lazy='joined',
        cascade='all, delete-orphan'
...
class RentalData(db.Model):
    ...
    location_name = db.Column(
        db.String(500),
        db.ForeignKey('location.name'),
        nullable=False,
        primary_key=True
    )
    location_rel = db.relationship(
        'Location', single_parent=True, back_populates='rental_data_rel'
    )
...
class DistanceMatrixData(db.Model):
    ...
    distance_to_london_text = db.Column(
        db.String(100), nullable=True, default=None
    )
    duration_to_london_text = db.Column(
        db.String(100), nullable=True, default=None
    )
    datetime = db.Column(
        db.DateTime, default=datetime.utcnow(), primary_key=True
...
class Scores(db.Model):
    ...
    location_rel = db.relationship(
        'Location', single_parent=True, back_populates='scores_rel'
    )
